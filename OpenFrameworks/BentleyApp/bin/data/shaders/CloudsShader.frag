...
void main()
{
    ...
    f = parameter + cloudalpha*f*r;
    
    vec3 result = mix(skycolour, clamp(skytint * skycolour + cloudcolour, 0.0, 1.0), clamp(parameter*(f + c), 0.0, 1.0));

    gl_FragColor = vec4(iColor.rgb*result, iColor.a );
}